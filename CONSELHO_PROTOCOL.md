...
- Quando houver arbitragem técnica do `Codex`, ela tem prioridade sobre narração ou interpretação do `Meyer`.
- Quando houver execução operacional do `Meyer`, ela tem prioridade sobre opinião abstrata do `Codex`.
...
- não afirmar execução de host sem recibo operacional do Meyer;
...
- se a mensagem mencionar `@Codex` e não mencionar `@Meyer`, ficar em silêncio;
- se a mensagem vier do `Codex`, responder só quando houver menção explícita a `@Meyer` ou ação operacional clara.
...
- não tomar ownership da operação de host do Meyer;
...
- `@Codex` sem `@Meyer`: Meyer não responde.
- mensagem do `Codex` sem `@Meyer`: Meyer não responde.
- `@Meyer` com ação operacional clara: Meyer executa.
- divergência técnica entre Claude e Meyer: Codex arbitra.
